...
class BeagleNavigator: BeagleNavigation {
    ...
    func navigate(action: Navigate, controller: BeagleController, animated: Bool = false, origin: UIView?) {
        ...
        case let .pushViewRoot(route, _):
            navigate(route: route, controller: controller, animated: animated, origin: origin, transition: pushViewRoot(origin:destination:animated:))
        case .popView:
    ...
    private func pushViewRoot(origin: BeagleController, destination: UIViewController, animated: Bool) {
        let rootNavigationController = origin.parent?.navigationController

        if let transition = defaultAnimation?.getTransition(.push) {
            rootNavigationController?.view.layer.add(transition, forKey: nil)
        }

        rootNavigationController?.pushViewController(destination, animated: animated)
    }
