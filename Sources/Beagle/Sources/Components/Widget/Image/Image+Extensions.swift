...
extension Image {
    ...
    private func observeFields(_ path: Image.ImagePath, _ renderer: BeagleRenderer, _ image: BeagleImageView) {
        ...
        case .local(let mobileId):
            ...
            renderer.observe(expression, andUpdateManyIn: image) { mobileId in
                guard let mobileId = mobileId, !mobileId.isEmpty else {
                    return
                }
    ...
    private func setImageFromAsset(named: String, bundle: Bundle, imageView: BeagleImageView) {
        imageView.image = UIImage(named: named, in: bundle, compatibleWith: nil)
    }

    private func setRemoteImage(from url: String, placeholder: String?, imageView: BeagleImageView, renderer: BeagleRenderer) -> RequestToken? {
    ...
    private func lazyLoadImage(path: String, placeholderImage: UIImage?, imageView: BeagleImageView, renderer: BeagleRenderer) -> RequestToken? {
...
private class BeagleImageView: FLAnimatedImageView {
