...
extension FirebaseUpdatePassword: AsyncAction {

    public func execute(controller: BeagleController, origin: UIView) {
        Auth.auth().currentUser?.updatePassword(to: password?.evaluate(with: origin) ?? "") { error in

            if let error = error {
                print("Update password errored: \(error)")
