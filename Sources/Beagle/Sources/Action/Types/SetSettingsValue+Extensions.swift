...
extension SetSettingsValue: AsyncAction {

    public func execute(controller: BeagleController, origin: UIView) {

        guard let keyEvaluated = key?.evaluate(with: origin),
              let valueEvaluated = value?.evaluate(with: origin) else {
        ...
        UserDefaults.standard.set(valueEvaluated, forKey: keyEvaluated)
